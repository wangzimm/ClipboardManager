"""Image clipboard tab — scrollable list of image thumbnail cards with batch ops."""
...
class ImageTab(QWidget):
    ...
    def _batch_copy(self):
        ...
        for item_id in self._selected_ids:
            card = self._cards.get(item_id)
            if card and card.item_type == "image":
    ...
    def _batch_delete(self):
        ...
        reply = QMessageBox.question(
            self, "确认删除", f"确定要删除选中的 {count} 张图片吗？\n删除后可在回收站保留 7 天。",
    ...
    def _delete_older_than(self, hours, label):
        reply = QMessageBox.question(
            self, "确认删除",
            f"确定要删除 {label} 之前的所有图片吗？\n（置顶项不会删除）\n删除后可在回收站保留 7 天。",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if reply == QMessageBox.StandardButton.Yes:
            from database import delete_items_older_than
            deleted = delete_items_older_than(hours, item_type="image")
            if deleted > 0:
                self._rebuild()
